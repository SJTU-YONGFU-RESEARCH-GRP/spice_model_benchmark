...
proc getCurrentParamValue_RNDIFF {param_name} {
	set val [db::getParamValue $param_name -of [getCurrentInstance_RNDIFF]]
	if {$val == ""} {
		set val 0
...
proc getLayers_RNDIFF {} {
	return [list\
		...
		M1 VIA1 M2 VIA2 M3 VIA3 M4 VIA4 M5 VIA5 M6 VIA6 M7 VIA7 M8 VIA8 M9 VIA0 HVTIMP LVTIMP\
		M1PIN M2PIN M3PIN M4PIN M5PIN M6PIN M7PIN M8PIN M9PIN MRDL VIARDL MRPIN DIOD BJTMY RNW\
		RMARK LOGO IP PrBoundary RM1 RM2 RM3 RM4 RM5 RM6 RM7 RM8 RM9 M1_3 M2_3 M3_3\
		#DM1EXCL DM2EXCL DM3EXCL DM4EXCL DM5EXCL DM6EXCL DM7EXCL DM8EXCL DM9EXCL VARMARK CBMMARK IPDMARK CTMMARK METDMY PWELL \
	]
}
...
proc is_digit_RNDIFF {val} {
	set value [ regsub -all {[^\.0-9]} $val "" ]
	return $value
}
...
proc set_Limits_RNDIFF {} {
	...
	return [list \
		...
		[list 1      500     1      500     1    $limit 0     1e100  0.1     $limit   0.28  $wf_limit 0.45   $lf_limit 0.1       $limit] \
	]
}
...
proc setInstanceParamValue_RNDIFF { param value inst} {
  ...
  proc get_param_def_RNDIFF { attr inst param} {
...
proc make_editable_RNDIFF {controlParamName MODE} {
	...
	set paramType [get_param_def_RNDIFF type [getCurrentInstance_RNDIFF] $controlParamName]
...
proc get_r {resistance_type l w fr rw} {
   ...
   set l [check_for_min_or_max_RNDIFF "lf" $l]
   set w [check_for_min_or_max_RNDIFF "wf" $w]
...
proc get_w {resistance_type l r fr rw} {
	...
	set w [check_grid_RNDIFF [check_for_min_or_max_RNDIFF "wf" [expr ($sheetR*($fr*$rw*$l*$um-$dl)) / (2*$r*$um) + $dw/$um]] 0.001]
	set new_w [check_for_min_or_max_RNDIFF "wf" $w]
	if {$w != $new_w} {
...
proc get_l {resistance_type w r fr rw} {
	...
	set dw   [get_dw_RNDIFF $resistance_type]
	set um	   [get_um_RNDIFF $resistance_type]
	set l [check_grid_RNDIFF [check_for_min_or_max_RNDIFF "lf" [expr ((2*$r*($w*$um-$dw)) / $sheetR + $dl)/($fr*$rw*$um)]] 0.001]
	set new_l [check_for_min_or_max_RNDIFF "lf" $l]
	if {$l != $new_l} {
...
proc is_numeric {value} {
	set val [ regsub -all {[^\.umnMY0-9]} $value "" ]
	return val
}	
...
proc to_user {value} {
	...
	if {[regexp {\d+k} $value]} {
		...
		return [expr [lindex [split [db::sciToEng $value] "YF"] 0] * pow(10, 24)] 
	} elseif {[regexp {\d+m} $value]} {
...
proc Rndiff14 {} { 
	set r [to_user [getCurrentParamValue_RNDIFF "r"]]

...
proc getLayers_ND {} {
	return [list\
		NWELL  DNW DIFF PIMP NIMP DIFF_18 DIFF_15 PAD ESD SBLK PO\
		M1 VIA1 M2 VIA2 M3 VIA3 M4 VIA4 M5 VIA5 M6 VIA6 M7 VIA7 M8 VIA8 M9 VIA0 CTM1 HVTIMP LVTIMP\
...
proc set_Limits_ND {} {
	return [list [list min_m min_w max_w min_l max_l] [list 1 0.28 50 0.45 50] ]
}

proc is_digit_ND {param_name param_val} {
...
proc setInstanceParamValue_ND { param value inst} {
	...
	if {[IsLayoutEditor]} {
		# param is a variable
		if { [is_variable_ND $formval] || [is_variable_ND $value] } {
			set old_val $formval
			set new_val $value
...
proc ND14 {} { 
	...
	set w 	 [to_user_ND  [is_digit_ND "w" [getCurrentParamValue_ND "w"]] "w"]
	setInstanceParamValue_ND "w" $w [getCurrentInstance_ND]
	set l    [to_user_ND  [is_digit_ND "l" [getCurrentParamValue_ND "l"]] "l"]
	setInstanceParamValue_ND "l" $l [getCurrentInstance_ND]

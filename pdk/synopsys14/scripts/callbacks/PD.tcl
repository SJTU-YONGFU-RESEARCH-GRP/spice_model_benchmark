...
proc setInstanceParamValue_PD { param value inst} {
	...
	if {[IsLayoutEditor]} {
		# param is a variable
		if { [is_variable_PD $formval] || [is_variable_PD $value] } {
			set old_val $formval
			set new_val $value
...
proc is_digit_PD {param_name param_val} {
	set value [regsub -all {[^\.0-9]} $param_val ""]
	if {$value == ""} {
	  set value [getLimitsValueByName_PD "min_${param_name}"]
	}
	return $value
}
...
proc PD14 {} { 
	set area    [getCurrentParamValue_PD "area"]]
	set m       [getCurrentParamValue_PD "m"]]
	set w 	    [is_digit_PD "w" [getCurrentParamValue_PD "w"]]
	setInstanceParamValue_PD "w" $w [getCurrentInstance_PD]
	set l	    [is_digit_PD "l" [getCurrentParamValue_PD "l"]]
	setInstanceParamValue_PD "l" $l [getCurrentInstance_PD]
	if {[IsLayoutEditor]} {
		set w $w
		set l $l
	} else {
		set w [lindex [split [db::sciToEng $w] "u"] 0]
		set l [lindex [split [db::sciToEng $l] "u"] 0]
	}
	...
	if {$edited_param == "w"} {
		...
		if {$w > $max_value_of_w} {
			...
			setInstanceParamValue_PD "${edited_param}" $w [getCurrentInstance_PD]
		}
		...
		if {$l > $max_value_of_l} {
			...
			setInstanceParamValue_PD "area" [db::sciToEng [get_Area_PD [expr $w*1e-6] [expr $l*1e-6]]] [getCurrentInstance_PD]
			setInstanceParamValue_PD "${edited_param}" $l [getCurrentInstance_PD]
		}

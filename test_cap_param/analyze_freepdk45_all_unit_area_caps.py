...
def load_summary(csv_path: Path):
    """Load freepdk45_all_unit_area_caps.csv into a structured dict.
...
def plot_unit_area_vs_temp(groups, out_dir: Path, pdk_name: str = "FreePDK45"):
...
def main():
    ...
    summary_csv = repo_root / "test_cap_param" / "results" / "freepdk45_all_unit_area_caps.csv"
    if not summary_csv.exists():
        raise FileNotFoundError(f"Summary CSV not found: {summary_csv}")

    groups = load_summary(summary_csv)

    plots_dir = repo_root / "test_cap_param" / "results" / "freepdk45" / "plots"
    plot_unit_area_vs_temp(groups, plots_dir, pdk_name="FreePDK45")

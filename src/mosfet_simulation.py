...
from spice_model_benchmark.simulation_runner import SimulationRunner
from spice_model_benchmark.data_reader import DataReader
from spice_model_benchmark.plot_generator import PlotGenerator
from spice_model_benchmark.verification_manager import VerificationManager
    ...
    def run(self, modes=['all']):
        ...
        try:
            ...
            if 'ac' in modes:
                ...
                # Produces:
                #   - data/ac_ls_caps_from_cv_integral.csv  (Cgg/Cgs/Cgd/Cgb + sweep path metadata)
                #   - data/ac_qg_from_cv_integral.csv       (Qg(Vg) integrated from Cgg(Vg))
                try:
                    ...
                    if vg_cv is not None and caps_cv is not None:
                        ...
                        if "Cgg" in caps_cv:
                            ...
                            for i_idx in range(1, vg_sorted.size):
                                dv_i = vg_sorted[i_idx] - vg_sorted[i_idx - 1]
                                qg[i_idx] = qg[i_idx - 1] + 0.5 * (cgg_sorted[i_idx] + cgg_sorted[i_idx - 1]) * dv_i
                            qg_c = qg
